...
PreparedStatement pst=null;
    ...
    private void initComponents() {
        ...
        jLabel4.setText("Hours of Overtime:");
                ...
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel2Layout.createSequentialGroup()
                        .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            ...
                            .addComponent(txt_rate, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, 126, Short.MAX_VALUE))
 ...
 try {
    ...
    // Calculate overall overtime rate
    dbop = salary / days / eight;
    
    // Format dbop to display only two decimal places
    String s = String.format("%.2f", dbop);
    
    // Set the formatted value to txt_rate
    txt_rate.setText(s);

    // Calculate total overtime pay
    double calc = Total_Overtime * dbop;

    // Add overtime pay to salary
    double totalSalary = salary + calc;

    // Format the total salary to display only two decimal places
    String formattedTotalSalary = String.format("%.2f", totalSalary);

    // Display the total salary
    lbl_total.setText(formattedTotalSalary);
} catch (NumberFormatException e) {
    JOptionPane.showMessageDialog(null, "Please enter valid numerical values for salary and overtime.");
} catch (Exception e) {
...
if (p == 0) {
    try {
        String hours = txt_overtime.getText();
        String percentage = txt_hw.getText(); // Assuming the overtime rate is always 1.25
        String totalOvertime = txt_rate.getText();
        String salaryOvertime = lbl_total.getText(); // Assuming total overtime is added to salary
        ...
        String checkIfExistsQuery = "SELECT * FROM Overtime WHERE id = ?";
        ...
        if (rs.next()) {
            ...
            String sql = "INSERT INTO Overtime (Hours, Percentage, Total_overtime, Salary_overtime, empid, firstname, surname, created_by, salary, emp_id, id) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

            pst = conn.prepareStatement(sql);
            pst.setString(1, hours);
            pst.setString(2, percentage);
            pst.setString(3, totalOvertime);
            pst.setString(4, salaryOvertime);
            ...
            pst.executeUpdate();
            JOptionPane.showMessageDialog(null, "Overtime Record Added");
            ...
            pst.setString(3, "Updated Overtime Record");
            pst.executeUpdate();
        }
...
Update_table();
    ...
    private void txt_emidActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txt_emidActionPerformed
        // TODO add your handling code here:
        try {
    String sql = "SELECT id, Hours, Percentage, Total_overtime FROM Overtime WHERE id=?";
    pst = conn.prepareStatement(sql);
    pst.setString(1, txt_search.getText());
    rs = pst.executeQuery();
    ...
    if (rs.next()) { // Check if the result set has data
        ...
        String add2 = rs.getString("Hours");
        txt_overtime.setText(add2);

        String add3 = rs.getString("Percentage");
        txt_hw.setText(add3);

        String add4 = rs.getString("Total_overtime");
        txt_rate.setText(add4);
...
if (p == 0) {
    try {
        ...
        if (rs.next()) {
            ...
            String sql = "UPDATE Overtime SET Hours = ?, Percentage = ?, Total_overtime = ?, Salary_overtime = ? WHERE id = ?";
            pst = conn.prepareStatement(sql);
            pst.setString(1, hours);
            pst.setString(2, percentage);
            pst.setString(3, totalOvertime);
            ...
            pst.executeUpdate();
            JOptionPane.showMessageDialog(null, "Overtime Record Updated");
            ...
            String reg = "INSERT INTO Audit (emp_id, date, status) VALUES (?, ?, ?)";
            pst = conn.prepareStatement(reg);
            pst.setString(1, id); // Log the ID of the employee whose record was updated
            pst.setString(2, value0 + " / " + values);
            pst.setString(3, "Updated Overtime Record");
            pst.executeUpdate();
        } else {
